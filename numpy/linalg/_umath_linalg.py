...
{pfx}zgemm{sfx}(char *transa, char *transb,
    ...
    class linearize_data(object):
        ...
        '''
        @lazy_init('row', 'columns', 'row_strides', 'column_strides')
        def __init__(*args):
            pass
    ...
    def wrap_slogdet(typ, basetyp, cblas_type, ctype):
        ...
        def slogdet(inout):
            ...
            linearize_matrix(mbuffer, inout[0], data, getattr(_C, cblas_type + 'copy'), ctype)
    ...
    def wrap_solvers(typ, cblas_type, ctype):
        def init_func(N, NRHS):
            ...
            pN = ffi.new('int32[1]', [N])
            pNRHS = ffi.new('int32[1]', [N])
            return gesv_params(A, B, ipiv, pN, pNRHS, pN, pN)

        def call_func(params):
            rv = ffi.new('int[1]')
            getattr(_C, cblas_type + 'gesv')(params.N, params.NRHS, params.A,
                                             params.LDA, params.IPV, params.B,
                                             params.LDB, rv)
            return rv[0]
        ...
        def solve(inout):
            ...
            n = inout[0].shape[0]
            nrhs = inout[0].shape[1]
            params = init_func(n, nrhs)
            a_in = linearize_data(n, n, inout[0].strides[1], inout[0].strides[0])
            b_in = linearize_data(nrhs, n, inout[1].strides[1], inout[1].strides[0])
            r_in = linearize_data(nrhs, n, inout[2].strides[1], inout[2].strides[0])
            copy_func = getattr(_C, cblas_type + 'copy')
            linearize_matrix(params.A, inout[0], a_in, copy_func, ctype) 
            linearize_matrix(params.B, inout[1], b_in, copy_func, ctype) 
            not_ok = call_func(params)
            if not_ok == 0:
                delinearize_matrix(inout[2], params.B, r_out, copy_func, ctype)
            else:
                error_occurred = 1
                inout[2].fill(base_vals[cblas_type]['nan'])
        ...
        def solve1(inout):
            ...
            params = init_func(n, nrhs)
            a_in = linearize_data(n, n, inout[0].strides[1], inout[0].strides[0])
            b_in = linearize_data(nrhs, n, 1, inout[1].strides[0])
            r_in = linearize_data(nrhs, n, 1, inout[2].strides[0])
            copy_func = getattr(_C, cblas_type + 'copy')
            linearize_matrix(params.A, inout[0], a_in, copy_func, ctype) 
            linearize_matrix(params.B, inout[1], b_in, copy_func, ctype) 
            not_ok = call_func(params)
        ...
        def inv(inout):
            ...
            a_in = linearize_data(n, n, inout[0].strides[1], inout[0].strides[0])
            r_in = linearize_data(n, n, inout[1].strides[1], inout[1].strides[0])
            copy_func = getattr(_C, cblas_type + 'copy')
            linearize_matrix(params.A, inout[0], a_in, copy_func, ctype) 
            identity_matrix(params.B)
            not_ok = call_func(params)
            if not_ok == 0:
                delinearize_matrix(inout[1], params.B, r_out, copy_func, ctype)
            else:
                error_occurred = 1
                inout[1].fill(base_vals[cblas_type]['nan'])
            set_fp_invalid_or_clear(error_occurred)

        return solve, solve1, inv

    FLOAT_solve,   FLOAT_solve, FLOAT_inv  = wrap_solvers(nt.float32,
                                                's', 'float*')
    DOUBLE_solve,  DOUBLE_solv, DOUBLE_inv  = wrap_solvers(nt.float64,
                                                'd', 'double*')
    CFLOAT_solve,  CFLOAT_solv, CFLOAT_inv  = wrap_solvers(nt.complex64,
                                                'c', "f2c_complex*")
    CDOUBLE_solve, CDOUBLE_solv, CDOUBLE_inv = wrap_solvers(nt.complex128,
                                                'z', 'f2c_doublecomplex*')
    ...
    solve = frompyfunc([FLOAT_solve, DOUBLE_solve, CFLOAT_solve, CDOUBLE_solve],
                         2, 1, dtypes=[nt.float32, nt.float32, nt.float32,
                          ...
                          signature='(m,m),(m,n)->(m,n)', name='solve', stack_inputs=False,
    ...
    solve1 = frompyfunc([FLOAT_solve1, DOUBLE_solve1, CFLOAT_solve1, CDOUBLE_solve1],
                         2, 1, dtypes=[nt.float32, nt.float32, nt.float32,
                                       ...
                                       nt.complex64, nt.float32, nt.float32,
                                       nt.complex128, nt.float64, nt.float64],
                          signature='(m,m),(m)->(m)', name='solve1', stack_inputs=False,
    ...
    inv = frompyfunc([FLOAT_inv, DOUBLE_inv, CFLOAT_inv, CDOUBLE_inv],
                         1, 1, dtypes=[nt.float32, nt.float32,
                                       ...
                                       nt.complex64, nt.float32,
                                       nt.complex128, nt.float64],
                          signature='(m,m)->(m,m)', name='inv', stack_inputs=False,
